...
app.use('/players', express.static(join(websiteDir, 'src/player')));
// Serve website content/images at /content so image URLs like /content/images/... work
app.use('/content', express.static(join(websiteDir, 'content')));
// Serve team page assets under /team
app.use('/teams', express.static(join(websiteDir, 'src/team')));
// Serve maps page assets under /map
app.use('/maps', express.static(join(websiteDir, 'src/map')));
// Serve compare page assets under /compare
app.use('/compare', express.static(join(websiteDir, 'src/compare')));
app.use('/home', express.static(join(websiteDir, 'src/home')));
app.use('/info', express.static(join(websiteDir, 'src/info')));

app.get('/', (req,res) => {
    res.sendFile(join(websiteDir, 'src/home/index.html'));
});
...
app.get('/api/players', async (req,res) => {
    try {
        const filters = {
            tournamentIds: req.query.tournamentIds || req.query.tournamentId,
            stages: req.query.stages || req.query.stage,
            regions: req.query.regions || req.query.region,
            teamIds: req.query.teamIds || req.query.teamId,
            roles: req.query.roles || req.query.role,
            circuits : req.query.circuits || req.query.circuits
        };
        const players = await db.getPlayerList(filters);
        res.json(players);
    } catch (error) {
...
app.get('/api/players/filters', async (req, res) => {
    try {
        const options = await db.getPlayerListFilterOptions();
        res.json(options);
    } catch (error) {
        console.error('Failed to fetch player filter options', error);
        res.status(500).json({ error: 'Unable to fetch player filter options' });
    }
})
    ...
    try {
        ...
        for (const row of h2hRows) {
            ...
            mapResults[row.map_id][teamKey] = {
                ...
                losses: row.losses,
                draws: row.draws,
                map_pick_count: row.map_pick_count || 0
            };
...
app.get('/api/players/:name', async (req, res) => {
    ...
    const result = {
        player_id: playerId,
        player_details: await db.getPlayerDetails(playerId),
        matches: await db.getPlayerMatchesById(playerId, tournamentIds, tournamentMode),
        stats: await db.getPlayerStats(playerId, tournamentIds, tournamentMode),
        total: await db.getTotalStats(playerId, tournamentIds, tournamentMode),
        avg: await db.getAvgStats(playerId, tournamentIds, tournamentMode),
        maps: await db.getPlayerMapStats(playerId, tournamentIds, tournamentMode),
        heroes: await db.getPreferredHeroes(playerId)
    };
    res.json(result);
});
...
app.get('/api/teams/:name', async (req, res) => {
    ...
    const result = {
        team_id: teamId,
        team_details: await db.getTeamDetails(teamId),
        matches: await db.getMatchesById(parseInt(teamId), tournamentIds, tournamentMode),
        roster: await db.getRoster(teamId),
        maps: await db.getTeamMapStats(teamId, tournamentIds, tournamentMode),
        bans: await db.getBanStats(teamId, tournamentIds, tournamentMode),
        tournaments: await db.getRecentTournaments(teamId, tournamentIds, tournamentMode)
    };
    res.json(result);
});
...
app.get('/players/:name', (req,res) => {
    ...
    res.sendFile(join(websiteDir, 'src/player/player.html'))
})
...
app.get('/teams/:name', (req, res) => {
    // normalize encoded spaces in path param
    req.params.name = decodeURIComponent(req.params.name).replace(/\+/g, ' ');
    res.sendFile(join(websiteDir, 'src/team/team.html'));
});

app.get('/players', (req,res) => {
    // normalize encoded spaces in path param
    res.sendFile(join(websiteDir, 'src/player/players.html'))
})

app.get('/maps', (req,res) => {
    res.sendFile(join(websiteDir, 'src/map/maps.html'))
})

app.get('/maps/:name', (req,res) => {
    // normalize encoded spaces in path param
    req.params.name = decodeURIComponent(req.params.name).replace(/\+/g, ' ');
    res.sendFile(join(websiteDir, 'src/map/map.html'))
})

app.get('/compare', (req,res) => {
    res.sendFile(join(websiteDir, 'src/compare/compare.html'))
})

app.get('/teams', (req,res) => {
    res.sendFile(join(websiteDir, 'src/team/index.html'))
})

app.get('/api', (req, res) => {
    res.sendFile(join(websiteDir, 'src/info/api.html'));
});

app.get('/about', (req, res) => {
    res.sendFile(join(websiteDir, 'src/info/about.html'));
});

app.get('/privacy', (req, res) => {
    res.sendFile(join(websiteDir, 'src/info/privacy.html'));
});

app.get('/terms', (req, res) => {
    res.sendFile(join(websiteDir, 'src/info/terms.html'));
});
