...
async function loadPlayerData() {
    ...
    const data = await fetch(`/api/players/${encodeURIComponent(playerIdentifier)}?${query}`)
        ...
        .catch(error => { console.warn('Failed to load player data', error); return null; });
    ...
    if (roleIconEl) {
        if (playerDetails.role) {
            roleIconEl.src = `/content/images/roles/${encodeURIComponent(String(playerDetails.role).trim().toLowerCase())}.webp`;
...
function renderStatChart(statName) {
    ...
    statChartData.forEach((match, index) => {
        ...
        labels.push(`${match.opponent} : ${date.toDateString()}` || `Match ${index + 1}`);
...
function renderMapResults() {
    ...
    const mapsToRender = sortMaps(
        currentMapView === 'mode' ? groupMapsByMode(playerMapData) : playerMapData
    );
