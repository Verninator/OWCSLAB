...
function renderRosterTable(team) {
    ...
    function sort_roster(players) {
        ...
        players.sort( function(a, b) {
            return (ordering[a.role] - ordering[b.role]) || a.player_name.localeCompare(b.player_name);
        });
        return players
} 
    ...
    team.players = sort_roster(team.players)
...
function getRegionIcon(region) {
    if (currentCircuit === 'owwc') {
        ...
        return `/content/images/regions/owwc/${owwcIcon.toLowerCase()}.webp`;
    }
    ...
    return `/content/images/regions/${iconName.toLowerCase()}.webp`;
}
...
function renderInactiveSection(inactiveTeams) {
    const teamCards = inactiveTeams.map(team => {
        ...
        const iconPath = normalizeTeamIconPath(team.team_icon || '/content/images/teams/default.png');
        return `
            <article class="team-box ${teamClass}">
                <div class="team-box-header">
                    <a href="/teams/${encodeURIComponent(team.team_name)}"><img src="${iconPath}" alt="${team.team_name}"></a>
                    <div>
        ...
        const teamCards = regionTeams.map(team => {
            ...
            return `
                <article class="team-box ${teamClass}">
                    ...
                    <button type="button" class="roster-toggle" data-roster-toggle aria-expanded="true">Hide roster</button>
                    <div class="team-roster-content" data-roster-content>
...
async function loadTeams() {
    const page = document.querySelector('.team-list-page');
