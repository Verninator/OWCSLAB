...
function buildHeroIconPath(icon) {
    ...
    const candidate = icon.startsWith('/') ? icon : `/content/${icon}`;
    return buildAssetPath(candidate);
}

function buildTeamIconPath(icon) {
    if (!icon) return '/content/images/teams/default.png';
    const candidate = icon.startsWith('/') ? icon : `/${icon}`;
    return buildAssetPath(candidate);
}
...
function buildTeamIcon(team, className = 'team-badge-icon') {
  const icon = `<img src="${buildTeamIconPath(team.icon)}" alt="${team.name}" title="${team.name}" class="${className}">`;
  return team.name ? `<a href="/teams/${encodeURIComponent(team.name)}" class="team-icon-link">${icon}</a>` : icon;
}
...
function buildStatsCard(title, stats) {
  return `
    <div class="stats-card">
      <h3>${title}</h3>
      <dl>
        ${Object.entries(stats).map(([key, value]) => { if (key == 'win_rate') {
          return `
          <div class="stat-row">
  ...
  const grouped = maps.reduce((acc, map) => {
    ...
    if (!acc[mode]) {
      acc[mode] = {
        ...
        map_icon: '',
        map_mode: mode,
        teamA: { played: 0, wins: 0, losses: 0, draws: 0, map_pick_count: 0 },
        teamB: { played: 0, wins: 0, losses: 0, draws: 0, map_pick_count: 0 }
      };
    }
    ...
    acc[mode].teamA.map_pick_count += parseStatValue(map.teamA.map_pick_count);
    ...
    acc[mode].teamB.map_pick_count += parseStatValue(map.teamB.map_pick_count);
...
function getPickRateForTeam(item, teamKey, teamData) {
  const teamStats = item[teamKey] || { played: 0, wins: 0, losses: 0, draws: 0, map_pick_count: 0 };
  const mapPickCount = parseStatValue(teamStats.map_pick_count);
  const modeKey = item.map_mode || 'Unknown';
  const modeMatches = !teamData
    ? []
    : teamData.maps.filter(mapEntry => (mapEntry.map_mode || 'Unknown') === modeKey);
  const modePickTotal = modeMatches.reduce((total, mapEntry) => total + parseStatValue((mapEntry[teamKey] || {}).map_pick_count), 0);

  return {
    percentage: modePickTotal ? Math.round((mapPickCount / modePickTotal) * 100) : 0,
    picks: mapPickCount
  };
}

function sortCompareMaps(items) {
  return [...items].sort((left, right) => String(left.map_name || '').localeCompare(String(right.map_name || '')));
}

function sortCompareBans(items) {
  return [...items].sort((left, right) => String(left.hero_name || '').localeCompare(String(right.hero_name || '')));
}


function buildMapComparisonSection(data, teamAColor, teamBColor) {
  const groupedItems = currentMapView === 'mode' ? groupMapsByMode(data.maps) : data.maps;
  const items = sortCompareMaps(groupedItems);
  const mapCards = items.map(item => {
    ...
    const teamAPick = getPickRateForTeam(item, 'teamA', data);
    const teamBPick = getPickRateForTeam(item, 'teamB', data);
    ...
    const mapBackgroundStyle = !useModeIcon && mapIconPath
      ...
      : '';
    const teamALine = `
      <span class="pill-team-block">
        <span class="pill-team-icon-column">${buildTeamIcon(data.teamA, 'hover-team-icon')}</span>
        <span class="pill-team-text-column">
          <span class="pill-team-line">
            <span class="pill-team-stat"><span class="pill-team-metric">WR ${teamAWinRate}</span></span>
            <span class="pill-team-separator">|</span>
            <span class="pill-team-stat pill-team-record"><span class="pill-team-metric">PR ${teamAPick.percentage}%</span></span>
          </span>
          <span class="pill-team-line pill-team-line-secondary">
            <span class="pill-team-stat pill-team-record"><span>${item.teamA.wins}/${item.teamA.losses}/${item.teamA.draws}</span></span>
            <span class="pill-team-separator">|</span>
            <span class="pill-team-stat pill-team-record"><span>Picked ${teamAPick.picks}</span></span>
          </span>
        </span>
      </span>
    `;
    const teamBLine = `
      <span class="pill-team-block">
        <span class="pill-team-icon-column">${buildTeamIcon(data.teamB, 'hover-team-icon')}</span>
        <span class="pill-team-text-column">
          <span class="pill-team-line">
            <span class="pill-team-stat"><span class="pill-team-metric">WR ${teamBWinRate}</span></span>
            <span class="pill-team-separator">|</span>
            <span class="pill-team-stat pill-team-record"><span class="pill-team-metric">PR ${teamBPick.percentage}%</span></span>
          </span>
          <span class="pill-team-line pill-team-line-secondary">
            <span class="pill-team-stat pill-team-record"><span>${item.teamB.wins}/${item.teamB.losses}/${item.teamB.draws}</span></span>
            <span class="pill-team-separator">|</span>
            <span class="pill-team-stat pill-team-record"><span>Picked ${teamBPick.picks}</span></span>
          </span>
        </span>
      </span>
    `;
...
function renderComparison(data) {
  ...
  const banItems = sortCompareBans(currentBanView === 'role' ? groupCompareBansByRole(heroItems) : heroItems);
  const banCards = banItems.map(item => {
    ...
    return `
      <div class="ban-pill">
        <span class="ban-pill-name">${item.hero_name}</span>
        <div class="ban-pill-body">
          ${iconMarkup}
          <div class="ban-pill-content">
            <strong>${totalBans} bans</strong>
            <div class="ban-pill-team-list">
              <span class="ban-pill-team-count">${teamABans}</span>
              <span class="ban-pill-team-count">${teamBBans}</span>
            </div>
          </div>
        </div>
      </div>
...
async function compareTeams() {
  ...
  try {
    compareResults.innerHTML = '<div class="loading-placeholder" role="status" aria-label="Loading"><span class="loading-spinner" aria-hidden="true"></span></div>';
    currentMapView = 'map';
    currentBanView = 'hero';
